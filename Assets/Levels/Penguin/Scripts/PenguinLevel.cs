...
public class PenguinLevel : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < sprites.Length; i++)
        {
            ...
            pinRenderer = sprites[i].GetComponent<MeshRenderer>();
            pinRenderer.sortingOrder -= i;
        }
    ...
    IEnumerator MoveHandler()
    {
        if(spriteCounter < howManySprites)
        {
            ...
            if (leftOrRight == 0)
            {
                chosenSprite.transform.localScale = new Vector3(-1, 1, 1);
                speed = -speed;
            }
            chosenSprite.GetComponent<PinAnime>().WalkAnime();
            move = true;
            spriteCounter += 1;
        }
        else
   ...
   public void LeftBtnPressed(int playerIndex)
    {
        if (!playedPlayers.Contains(playerIndex) && chosenSprite)
        {
            ...
            {
                audioManager.GetComponent<Music>().PlaySound(0);
                if(playerIndex == 0)
                {
                    //ax emtiyaz
            ...
            {
                audioManager.GetComponent<Music>().PlaySound(1);
            }
        }
    }
